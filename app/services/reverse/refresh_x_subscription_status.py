"""
Reverse interface: refresh X subscription status.
"""
...
REFRESH_X_SUBSCRIPTION_STATUS_API = (
    "https://grok.com/rest/auth/refresh-x-subscription-status"
)


class RefreshXSubscriptionStatusReverse:
    """/rest/auth/refresh-x-subscription-status reverse interface."""
    ...
    async def request(session: AsyncSession, token: str) -> Any:
        try:
            ...
            logger.error(
                f"RefreshXSubscriptionStatusReverse: Request failed, {type(e).__name__}: {e}"
            )
            raise UpstreamException(
                message=f"RefreshXSubscriptionStatusReverse: Request failed, {e}",
                details={"status": 502, "error": str(e)},
            )


__all__ = ["RefreshXSubscriptionStatusReverse"]
